...
from tkinter import *
from EvaluateComplexExpressions import *
...
def press(num):
    # point out the global expression variable ; to change the value of a global variable inside a function
    global expression
...
def equalpress():
    ...
    try:
        ...
        express_tree = ExpressionTree(expression)

        total = evaluate_expression_tree(express_tree.root)
...
if __name__ == "__main__":
    ...
    expression_field.grid(columnspan=3, ipadx=55)
    ...
    lparenthes = Button(gui, text=' ( ', fg='white', bg='blue',
                   command=lambda: press(" ( "), height=1, width=7)
    lparenthes.grid(row=7, column=0)

    rparenthes = Button(gui, text=' ) ', fg='white', bg='blue',
                        command=lambda: press(" ) "), height=1, width=7)
    rparenthes.grid(row=7, column=2)
